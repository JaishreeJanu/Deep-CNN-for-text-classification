...
class VDCNN(object):
    ...
    """
    def __init__(self, num_classes=2, cnn_filter_size=3, pooling_filter_size=2, num_filters_per_size=["64","128","256","512"],num_rep_block=(10,10,4,4), num_quantized_chars=70, sequence_max_length=144, l2_reg_lambda=0.0):

        ## num_filters_per_size=(64,128,256,512) changed to    num_filters_per_size="64,128,256,512"
        ...
        with tf.name_scope("first-conv-layer"):
            filter_shape = [cnn_filter_size, num_quantized_chars, 1, num_filters_per_size[0]]	  ## [filter_height, filter_width, in_channels, out_channels]
            ...
            with tf.name_scope("conv-block"):
                ...
                for j in range(0,num_rep_block[i]):
                    ...
                    with tf.name_scope("sub1"):
                        ...
                        batch_norm1 = tf.contrib.layers.batch_norm(conv1, center=True, scale=True, decay=0.9, is_training=self.is_training)	 ## removed name='bn1'
                    ...
                    with tf.name_scope("sub2"):
                        ...
                        batch_norm2 = tf.contrib.layers.batch_norm(conv2, center=True, scale=True, decay=0.9, is_training=self.is_training)   ## removed name='bn2'
                        h = tf.nn.relu(batch_norm2, name="relu2")
                        
                        
            if (i!=3):	# don't do max pooling at the last conv block
                with tf.name_scope("max-pooling"):
                    h = tf.nn.max_pool(h, ksize=[1, 1, pooling_filter_size, 1],strides=[1, 1, 2, 1], padding='VALID', name="pool")
